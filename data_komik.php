...
<div class="w3-main" style="margin-left:250px">
  ...
  include "koneksi.php";
    ...
    if(isset($_GET['id_komik'])){
        $id= $_GET['id_komik'];
        mysqli_query($connect,"DELETE FROM komik WHERE id_komik='$id'");
    }

    $query = mysqli_query($connect,"SELECT * FROM vdatakomik");
    //echo "<h3><b># KOMIK</b></h3>";
    echo "<table border=1 >";
    echo "<tr><th>No</th><th>ID Komik</th><th width=150px>Judul Komik</th><th>Genre</th><th>Rating</th><th>Status</th><th>Sinopsis</th><th>Edit</th><th>Hapus</th></tr>";

    $noo=1;
    while($kolom = mysqli_fetch_array($query)){
        $id = $kolom["id_komik"];
        $judul = $kolom["judul_komik"];
        $genre_komik = $kolom["nama_genre"];
        $rating = $kolom["rating"];
        $status = $kolom["status"];
        $sinopsis = $kolom["sinopsis"];
        $hapus = "<a href=?id_komik=$id>Hapus</a>";
        $edit = "<a href=edit.php?id_komik=$id>Edit</a>";
        echo "<tr><td align=center>$noo</td><td>$id</td><td>$judul</td><td>$genre_komik</td><td align=center>$rating</td><td align=center>$status</td><td>$sinopsis</td><td>$edit</td><td>$hapus</td></tr>";
        $noo++;
    }
  ...
  ?>
      <header class="w3-display-container w3-wide" id="home">
        ...
        <h1><a href="tambah_komik.php"><button class="w3-button w3-black w3-padding-small w3-large w3-opacity w3-hover-opacity-off">Tambahkan Data</button></a></h1>
        </div>
      </header>
    </div>
